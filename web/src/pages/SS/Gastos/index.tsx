...
export default function SSGastos() {
  ...
  const insights: Insight[] = loadingCaps || capsOp.length === 0 ? [] : [
    ...(cap4 && totalOp > 0 ? [{
      label: 'Peso prestaciones sociales (cap. 4)',
      value: `${((cap4.importe / totalOp) * 100).toLocaleString('es-ES', { maximumFractionDigits: 1 })}%`,
      trend: 'neutral' as const,
      description: `Las transferencias corrientes (${formatEur(cap4.importe)}) incluyen pensiones y prestaciones por desempleo, incapacidad y otras contingencias. Son el mayor bloque del gasto de la Seguridad Social.`,
    }] : []),
    ...(yoyRatio != null ? [{
    ...
    ...(cap1 && totalOp > 0 ? [{
      ...
      trend: 'neutral' as const,
      description: `El capítulo 1 recoge nóminas y cotizaciones sociales de los empleados de la Seguridad Social.`,
    }] : []),
  ]
    ...
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle={isComparativa
          ? `Seguridad Social · Plan vs Ejecución ${selectedYear}`
          : `Seguridad Social · ${fuente === 'plan' ? 'Plan' : 'Ejecución'} ${selectedYear}`}
      />

      <ContextBox title="Clasificación del gasto de la Seguridad Social">
        <p>
          El gasto de la Seguridad Social se clasifica según su <strong>naturaleza económica</strong>.
          El <strong>capítulo 4</strong> (Transferencias corrientes) concentra la mayor parte:
          pensiones contributivas, prestaciones por desempleo, incapacidad temporal y otras
          prestaciones económicas.
        </p>
        <p>
          El <strong>capítulo 2</strong> recoge los gastos corrientes en bienes y servicios,
          fundamentalmente la atención sanitaria prestada a través del INGESA y otros organismos.
          Los datos de ejecución proceden del IGAE y están disponibles desde 2015.
        </p>
      </ContextBox>
      ...
      {!isComparativa && (
        <>
          ...
          <section>
            <div className="chart-card-rule bg-white border border-[var(--color-rule)] px-5 pt-4 pb-2">
              <h2 className="text-sm font-semibold text-[var(--color-ink)] mb-1">
                Evolución del gasto por capítulo · Seguridad Social
              </h2>
              <p className="text-xs text-[var(--color-ink-muted)] mb-4">
