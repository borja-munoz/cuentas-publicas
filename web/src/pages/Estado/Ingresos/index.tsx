...
export default function EstadoIngresos() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Ingresos"
        subtitle={`Estado · ${isComparativa ? `Plan vs Ejecución ${selectedYear}` : `${fuente === 'plan' ? 'Plan' : 'Ejecución'} ${selectedYear}`}`}
      />

      {!isComparativa && (
        <ContextBox title="Ingresos del Estado">
          <p>
      ...
      {!isComparativa && (
        <>
          ...
          <section>
            <div className="chart-card-rule bg-white border border-[var(--color-rule)] px-5 pt-4 pb-2">
              <h2 className="text-sm font-semibold text-[var(--color-ink)] mb-1">
                Evolución de ingresos no financieros · Estado
              </h2>
              <p className="text-xs text-[var(--color-ink-muted)] mb-4">
