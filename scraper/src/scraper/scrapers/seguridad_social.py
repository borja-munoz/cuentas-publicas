"""
Scraper Seguridad Social — Presupuesto SS (2014–2025).

Fuente: SEPG, misma estructura de ficheros que el Estado.
URL base: .../Estadisticas/Documents/{YEAR}/03 Presupuesto de la Seguridad Social.xlsx

Mismo formato wide que el Estado (hojas "21" gastos, "23" ingresos).
Valores en millones de euros. entidad = 'SS'.
"""
...
_FILE_CANDIDATES = [
    "03%20Presupuesto%20de%20la%20Seguridad%20Social.xlsx",
    "03%20Presupuestos%20de%20la%20Seguridad%20Social.xlsx",
    "03%20Seguridad%20Social.xlsx",
]
...
# ---------------------------------------------------------------------------
# Mapeo capítulo-etiqueta → número (específico para SS)
# ---------------------------------------------------------------------------
...
def _parse_file(data: bytes) -> tuple[list[dict], list[dict]]:
    ...
    # SS usa hojas "31" (gastos) y "32" (ingresos)
    gastos_sheets = [s for s in xl.sheet_names if re.match(r"^3\.?1$", str(s).strip())]
    ...
    ingresos_sheets = [s for s in xl.sheet_names if re.match(r"^3\.?2$", str(s).strip())]
...
def run(conn: duckdb.DuckDBPyConnection, year: int | None = None) -> None:
    """Descarga y carga el presupuesto de la Seguridad Social (SEPG)."""
    ...
    if gastos_by_year:
        ...
        if not df_g.empty:
            ...
            n = upsert(
                ...
                "gastos_plan",
                df_g,
                delete_where=f"entidad = 'SS' AND year IN ({years_str})",
            )
            console.print(f"  → gastos_plan (SS): {n} filas.")
    ...
    if ingresos_by_year:
        ...
        if not df_i.empty:
            ...
            console.print(f"  → ingresos_plan (SS): {n} filas.")
